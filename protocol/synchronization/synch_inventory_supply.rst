...
under the URL contained in the ``supplyInventoryUrl`` field, as defined in the :ref:`Metadata Format <protocol-synchronization-metadata>`.

The URL published under the ``supplyInventoryUrl`` field should support the ``limit`` and ``offset`` query parameters for pagination.
